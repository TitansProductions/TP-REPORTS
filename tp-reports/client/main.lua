...
RegisterNetEvent('tp-reports:openReportNUI')
AddEventHandler('tp-reports:openReportNUI', function()
	if not isDead then
...
RegisterCommand('report', function(source, args) 
  ...
  if args and #args > 0 then
    ...
    TriggerEvent('chat:addMessage', {color = { 255, 0, 0},multiline = true,args = {Config.ServerNamePrefix, " Please, use - /report to submit a report."}})
    return
  end
...
RegisterCommand('reports', function(source, args) 
	...
	ESX.TriggerServerCallback("tp-reports:fetchUserGroup", function(playerGroup)
        if Config.WhitelistedGroups[playerGroup] then
			...
			if args and #args > 0 then
				TriggerEvent('chat:addMessage', {color = { 255, 0, 0},multiline = true,args = {Config.ServerNamePrefix, " Please, use - /reports to submit a report."}})
				return
			  end
			  ...
			  if not isDead then
				...
				ESX.TriggerServerCallback('tp-reports:getReports', function(cb_reports)
					ESX.TriggerServerCallback('tp-reports:getSolvedReportsCount', function(solvedReports)
						Wait(500)
...
RegisterNUICallback('refreshReports', function()
	refreshReports()
end)
...
RegisterNUICallback('refreshSolvedReports', function()

	SendNUIMessage({
		action = 'clearReports',
	})

	ESX.TriggerServerCallback('tp-reports:getSolvedReports', function(cb_solvedreports)

		for k,v in pairs(cb_solvedreports)do
			SendNUIMessage({
				action = 'addSolvedReport',
				reports_det = v,
				reports_id  = k,
			})		
